...
class Map(object):
    ...
    def start(self, s):
        self._start = s
        self.draw_end_start(self._start, 'o', color='green')
    ...
    def draw_end_start(self, p, shape, color):
        plt.plot(p[0], p[1], shape, color=color, markersize=10)
        if len(p) == 3:
            arrow_length = self.width / 20.0
            plt.arrow(p[0], p[1], arrow_length * math.cos(p[2]), arrow_length * math.sin(p[2]), head_width=arrow_length/5)
    @property
    ...
    def end(self, e):
        self._end = e
        self.draw_end_start(self._end, 'o', color='red')
    ...
    @property
    def obstacle_box(self):
        return self._obstacle_box

    @obstacle_box.setter
    def obstacle_box(self, o):
        self._obstacle_box = copy.deepcopy(o)
    ...
    def roadmap(self, o):
        ...
        t = list(zip(*self.roadmap.keys()))
        plt.plot(t[0], t[1], '.', color='blue')
        self.update()
        for k, v in self.roadmap.items():
            for p in v:
                plt.plot([k[0], p[0]], [k[1], p[1]], color='lightblue', alpha=0.3)
        plt.plot(t[0], t[1], '.', color='blue')
        self.update()
    ...
    def path(self, o):
        ...
        plt.plot(t[0], t[1], color='purple', alpha=0.5)
